...
char *
sodium_bin2hex_morello(char *__capability const hex, const size_t hex_maxlen,
               const unsigned char *__capability const bin, const size_t bin_len)
{
    size_t       i = (size_t) 0U;
    unsigned int x;
    int          b;
    ...
    return (__cheri_fromcap char*)hex;
}
               ...
               const unsigned char *const bin, const size_t bin_len)
{
    char *ret;
    switch_to_comp1++;
    __flexos_morello_gate4_r_cici(0, 1, ret, sodium_bin2hex_morello, (char *__capability)hex, hex_maxlen, (char *__capability)bin, bin_len);
    return ret;
}

int
sodium_hex2bin_morello(unsigned char *__capability bin, const size_t bin_maxlen,
               const char *__capability hex, const size_t hex_len,
               const char *__capability ignore, size_t *__capability bin_len,
               const char **__capability hex_end)
{
    ...
    while (hex_pos < hex_len) {
        ...
        if ((c_num0 | c_alpha0) == 0U) {
            // cheri_bounds_set((__cheri_tocap __typeof__(char) *__capability)strchr((__cheri_fromcap)ignore, c), sizeof(char))
            if (ignore != NULL && state == 0U && strchr((__cheri_fromcap const char*)ignore, c) != NULL) {
                while(1);
                hex_pos++;
                continue;
            }
    ...
    if (hex_end != NULL) {
        *hex_end = (__cheri_fromcap const char *)&hex[hex_pos];
               ...
               const char **const hex_end)
{
    int ret;
    switch_to_comp1++;
    __flexos_morello_gate7_r(0, 1, ret, sodium_hex2bin_morello, (char* __capability)bin, bin_maxlen, (char *__capability)hex, hex_len, (char *__capability)ignore, (size_t *__capability)bin_len, (char **__capability)hex_end);
    return ret;
}
