...
static void
store32_le_morello(uint8_t __capability* dst, uint32_t w)
{
    dst[0] = (uint8_t) w; w >>= 8;
    dst[1] = (uint8_t) w; w >>= 8;
    dst[2] = (uint8_t) w; w >>= 8;
...
{
    __flexos_morello_gate2_ci(0, 1, store32_le_morello, (uint8_t *__capability)dst, w);
}
...
static inline void
store64_be_morello(uint8_t *__capability dst, uint64_t w)
{
    dst[7] = (uint8_t) w; w >>= 8;
    dst[6] = (uint8_t) w; w >>= 8;
    dst[5] = (uint8_t) w; w >>= 8;
...
{
    __flexos_morello_gate2_ci(0, 1, store64_be_morello, (uint8_t *__capability)dst, w);
}
